...
export default function Layout() {
  ...
  useEffect(() => {
    if (location.pathname === '/report' || location.search.includes('report')) {
          ...
          <nav className="flex items-center gap-1.5 sm:gap-3">
            {navItems.map(({ to, label, icon: Icon, end }) => (
              <NavLink
                ...
                className={({ isActive }) =>
                  `relative flex items-center gap-2.5 px-4 sm:px-5 h-12 rounded-2xl font-hero font-bold text-xs sm:text-sm tracking-wide transition-colors ${
